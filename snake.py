...
def snake_ate_food(snake, food):
    ...
    """
    for indicator in range(len(snake)):
        if snake[indicator] == food:
            return True
...
def snake_ran_out_of_bounds(snake):
    ...
    """
    for side in range(len(snake)):
        for sides in range(len(snake[side])):
            if snake[side][sides] == 30:
                return True
            elif snake[side][sides] == -1:
                return True 
    return False
...
def snake_intersected_body(snake):
    ...
    """
    for snake_ran in range(1,len(snake)):
        if snake[snake_ran] == snake[0]:
            return True
    return False
